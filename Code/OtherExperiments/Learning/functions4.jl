...
function compute_Pxbinary(x,W,b,c)
    ...
    Px = Base.MathConstants.e.^((b'*x) .+ sum(log.(1 .+ exp.(W*x .+ c)), dims = 1));
end
...
function computeRBMparamAll(W, b, c, I, I_outside, Nexamples, batchsize, NG, Nepochs, momentum, targdis, plotLL)
    ...
    # W_WCD    = copy(W); b_WCD   = copy(b); c_WCD   = copy(c); ratio_WCD   = [0, 0]; VdW_WCD   = copy(VdW_CD1); Vdb_WCD   = copy(Vdb_CD1); Vdc_WCD   = copy(Vdc_CD1);
    ...
    # W_PTWCD  = copy(W); b_PTWCD = copy(b); c_PTWCD = copy(c); ratio_PTWCD = [0, 0]; VdW_PTWCD = copy(VdW_CD1); Vdb_PTWCD = copy(Vdb_CD1); Vdc_PTWCD = copy(Vdc_CD1);
    # W_CDP    = copy(W); b_CDP   = copy(b); c_CDP   = copy(c); ratio_CDP   = [0, 0]; VdW_CDP   = copy(VdW_CD1); Vdb_CDP   = copy(Vdb_CD1); Vdc_CDP   = copy(Vdc_CD1);
    # W_WCDP   = copy(W); b_WCDP  = copy(b); c_WCDP  = copy(c); ratio_WCDP  = [0, 0]; VdW_WCDP  = copy(VdW_CD1); Vdb_WCDP  = copy(Vdb_CD1); Vdc_WCDP  = copy(Vdc_CD1);
    # W_FIN   = copy(W) ; b_FIN   = copy(b); c_FIN   = copy(c); ratio_FIN   = [0, 0]; VdW_FIN   = copy(VdW_CD1); Vdb_FIN   = copy(Vdb_CD1); Vdc_FIN   = copy(Vdc_CD1);
    ...
    # expdis_CD1   = compute_Pxbinary(I, W_CD1  , b_CD1  , c_CD1  );
    # expdis_CD    = compute_Pxbinary(I, W_CD   , b_CD   , c_CD   );
    # expdis_WCD   = compute_Pxbinary(I, W_WCD  , b_WCD  , c_WCD  );
    ...
    # expdis_PTWCD = compute_Pxbinary(I, W_PTWCD, b_PTWCD, c_PTWCD);
    # expdis_CDP   = compute_Pxbinary(I, W_CDP  , b_CDP  , c_CDP  );
    # expdis_WCDP  = compute_Pxbinary(I, W_WCDP , b_WCDP , c_WCDP );
    # expdis_FIN   = compute_Pxbinary(I, W_FIN  , b_FIN  , c_FIN  );
    params = Dict(
    ...
    plotparams = Dict(
                      ...
                      # "ratio_CD"    => ratio_CD   ,
                      # "ratio_WCD"   => ratio_WCD  ,
                      "ratio_PTCD"  => ratio_PTCD ,
                      # "ratio_PTWCD" => ratio_PTWCD,
                      # "ratio_CDP"   => ratio_CDP  ,
                      # "ratio_WCDP"  => ratio_WCDP ,
                      # "ratio_FIN"   => ratio_FIN
                      );
...
function myLogSumExp(X)
    # Equivalent to log(exp(1)+exp(3)+exp(2))
    max_X = maximum(X);
